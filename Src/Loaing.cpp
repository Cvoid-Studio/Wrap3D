...
FiberStruct::FiberStruct()
{
	...
	memset(OutVer, 0, Max_Node * sizeof(Grid_node));

	memset(DeleteBox, 0, CutIndex * sizeof(FiberFace));
	memset(CutCri, 0, MaxFaces * sizeof(bool));

	for (long i = 0; i < MaxFaces; ++i)
	{
		Indexes[i] = 1;		// Accessible
	}
...
FiberStruct::~FiberStruct()
{
	memset(DataFace, NULL, MaxFaces * sizeof(FiberFace));
}

// =============================================================== //
int FiberStruct::ImportFiberDir(const char* filename, FiberFace* Target)
{
	using namespace std;
	cout << "reading faces...";
...
int FiberStruct::WriteFiberDir(const char* filename, FiberFace* Src)
{
	using namespace std;
...
int FiberStruct::WriteFiberVer(const char * filename, Scan_Grid* Src)
{
	using namespace std;
	ios_base::openmode ofs = ios_base::out;
	//-----------------------------------------------------------//
	...
	unsigned int Index_Mem = 0;
	char tempstr[16];
	while (Index_Mem < Max_Node)
	{
		for (int Layer = 0; Layer < Src[Index_Mem].Layers; ++Layer)
		{
			switch (Index_Dir)
			{
			case 0:
